...
public class Weapon extends Item { //add stamina reduction
	...
	public static final class Weapons { //make methods
		...
		public static Weapon rustyDagger() {
			return new Weapon("Rusty Dagger", "%1$s drive a rusty dagger into %2$s", (3 + (int)(Math.random()*3)), 2, 2, 2, .3, .2, new ArrayList<Probability<StatusEffect>>(Arrays.asList(new Probability<StatusEffect>(new StatusEffect("weakness", 2, 3), .3))), null);
		}
